# Redis

- [AWS Redis 정의](https://aws.amazon.com/ko/elasticache/what-is-redis/)
- [AWS Redis 정의2](https://aws.amazon.com/ko/redis/)
- [AWS Redis - ElastiCache](https://docs.aws.amazon.com/ko_kr/AmazonElastiCache/latest/UserGuide/WhatIs.html)
- [ElastiCache 실행정리](https://blog.wisen.co.kr/?p=174)

- [Spring Boot에서 Redis 사용하기](http://kingbbode.tistory.com/25)
- [스프링 공식문서 Spring Data Redis](https://docs.spring.io/spring-data/redis/docs/2.0.3.RELEASE/reference/html/)

- [mac에서 redis설치](http://compunication.tistory.com/20)

